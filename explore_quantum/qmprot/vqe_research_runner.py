#!/usr/bin/env python3
"""
VQE Research Runner - Complete Workflow
...
def setup_gpu_environment():
    ...
    try:
        ...
        if gpu_devices:
            ...
            logger.info("⚠ JAX available but no GPU detected, using CPU")
            return False
    except ImportError:
        ...
        try:
            ...
            logger.warning("Neither JAX nor TensorFlow GPU support available")
            return False
...
def run_excited_states_calculation(molecular_system: MolecularSystem,
    ...
    # Log results
    if 'error' not in excited_results:
        logger.info(f"✓ Calculated {len(excited_results['excited_states'])} excited states:")
        for i, (energy, _) in enumerate(excited_results['excited_states']):
            transition_energy = energy - ground_energy
            logger.info(f"  Excited state {i+1}: {energy:.6f} Ha (ΔE = {transition_energy:.6f} Ha)")
...
def main():
    ...
    parser.add_argument('--excited-method', default='deflation', 
                       choices=['subspace_expansion', 'deflation', 'qeom'],
